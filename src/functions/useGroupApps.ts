...
export default function groupByAppAndCountry(data: any) {
  ...
  for (const entry of data) {
    ...
    if (!map.has(`${app} ${os}`)) {
      ...
      appData[`totalRevenues${country}`] =
        (appData[`totalRevenues${country}`] || 0) + revenue;
      appData[`totalRevenues`] = (appData[`totalRevenues`] || 0) + revenue;
      appData[`totalViews`] = (appData[`totalViews`] || 0) + entry.views;
      appData[`totalConversions`] =
        (appData[`totalConversions`] || 0) + entry.conversions;
      appData[`platform`] = os == "Android" ? "android" : "ios";
      appData[`rewarded`] =
        (appData[`rewarded`] || 0) +
        (format == AdFormat.REWARDED ? revenue : 0);
      appData[`video`] =
        (appData[`video`] || 0) + (format == AdFormat.VIDEO ? revenue : 0);
      appData[`banner`] =
        (appData[`banner`] || 0) + (format == AdFormat.BANNER ? revenue : 0);
      appData[`fullscreen`] =
        (appData[`fullscreen`] || 0) +
        (format == AdFormat.FULLSCREEN ? revenue : 0);
    }
  }
